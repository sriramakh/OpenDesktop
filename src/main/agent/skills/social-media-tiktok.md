...
## Known Issue: Feed Author Selector
[Learned: 2026-04-06] Feed was returning empty author because sidebar profile link matched first

In the fullscreen logged-in layout,  is the primary author selector. Fallback  must skip nav/sidebar links by checking .

## Troubleshooting: Posted Videos Not Loading
[Learned: 2026-04-06] Discovered that TikTok profile page has issues with automation - need JavaScript click approach for Videos tab

## Problem: Posted Videos Not Loading

When navigating to `@me` or `@me/videos`, TikTok's profile page often doesn't load posted videos properly with automation tools. The page appears to redirect or shows empty content.

### Symptoms
- `social_read_profile` returns empty username/bio
- `social_read_feed` returns empty or partial posts
- Page URL may redirect to `/foryou`

### Solution: Click "Videos" Tab via JS

Use this JavaScript approach to click the Videos tab on your own profile:

```javascript
(() => {
  // Find the Videos tab by looking for h2/div elements with that text
  const elements = document.querySelectorAll('h2, div, a, span');
  for (const tab of elements) {
    if (tab.textContent.trim() === 'Videos') {
      tab.click();
      return 'Clicked Videos tab';
    }
  }
  return 'Videos tab not found';
})();
```

After clicking, wait 2-3 seconds then use `social_scroll` and `social_read_feed`.

### Alternative: Direct Tab Navigation

Try navigating directly using these URL patterns:
- `https://www.tiktok.com/@me` (profile home)
- Tab 11 in Chrome seems to work better than others for profile access

### If All Else Fails

Check the activity log for recent posts instead of trying to load the profile page directly.